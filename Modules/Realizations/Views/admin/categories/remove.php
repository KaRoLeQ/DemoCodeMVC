...
<?= $this->section('title') ?><?=lang('Hotel.removing_rooms_category_title').' #'.$category->id?><?= $this->endSection('') ?>
    ...
    $(document).delegate("form#removeCategoryForm", "submit", function(c) {
        ...
        if(form.get('remove')=='true')
        {
            form.append("categoryId", <?=$category->id?>);
                $.ajax({
                type: 'POST',
                url: '/admin/hotel/rooms/removeCategory',
                data: form,
                processData: false,
                contentType: false,
                success: function (data) {
                    ...
                    }else
                    {
                        ...
                        $(`#removeCategoryForm #formRaport`).addClass(`alert-success`).append(`<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h5><i class="icon fas fa-check"></i> <?=lang('OvenCms.success')?></h5>`).append('<?=lang('Hotel.remove_rooms_category_return_true_alert')?></h5>');
                        setTimeout(function(){
                            window.location.href = '/admin/hotel/rooms/categories';
                        }, 3100);
        ...
        {
            $(`#removeCategoryForm #formRaport`).addClass(`alert-success`).append(`<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h5><i class="icon fas fa-check"></i> <?=lang('OvenCms.success')?></h5>`).append('<?=lang('Hotel.remove_rooms_category_return_false_alert')?></h5>');
        }
...
<div class="content-header">
    ...
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark"><?=lang('Hotel.removing_rooms_category')?></h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item"><?=anchor('/admin/hotel/rooms/category/'.$category->id, $category->name)?></li>
            <li class="breadcrumb-item active"><?=anchor('/admin/blog/category/'.$category->id.'/remove', lang('OvenCms.remove'))?></li>
        </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
    </div><!-- /.container-fluid -->
        ...
        <div class="row">
            <div class="col-md-5">
                <div class="callout callout-warning">
                    <h5><?=lang('Hotel.remove_rooms_category_information_heading')?></h5>

                    <p><?=lang('Hotel.remove_rooms_category_information_1')?></p>
                    <p><?=lang('Hotel.remove_rooms_category_information_2')?></p>
                </div>
                <div class="card card-warning card-outline">
                    <div class="card-header">
                        <h3 class="card-title"><i class="fas fa-edit"></i> <?=lang('Hotel.form_removing_rooms_category');?></h3>
                    </div>
                    <!-- /.card-header -->
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <div class="form-check">
                                    ...
                                    <label for="removeTrue" class="form-check-label"><?=lang('Hotel.remove_rooms_category_yes_in_form')?></label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" id="removeFalse" name="remove" value="false" checked>
                                    <label for="removeFalse" class="form-check-label"><?=lang('Hotel.remove_rooms_category_no_in_form')?></label>
                                </div>
                            </div>
                        </div>
